...
public class UnsetPinCommand implements CommandExecutor {

    private final ShieldAuth plugin;

    public UnsetPinCommand(ShieldAuth plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!plugin.getDatabaseManager().hasPin(player.getUniqueId())) {
            player.sendMessage(plugin.getConfigManager().getPrefixedMessage("messages.pin-not-set"));
            return true;
        }
